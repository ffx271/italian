...
Pronouns attach to a (slightly modified form of the) \itb{infinitive}. They can, grammatically,
go at the start of the phrase too, but the former is considered more natural:
...
The modification is the drop the \itb{-e} from the infinitive and add the pronoun (\iti{mi, ti, lo, ci, vi, si}).

\chapter{Reflexive Verbs}
Reflexive verbs describe actions done \itb{to oneself}. Their infinitives end in \itb{-si}. They are
conjugated like regular \iti{-are, -ere, -ire} verbs. For example, \iti{alzare} in the third
person singular is \iti{si} + \iti{alza}.
...
For example: \iti{Si alza e si lava, poi, si siede al tavolo.}
...
\textbf{Note} Never drop the reflexive pronoun: \iti{si sono alzate}, \textbf{not} \textit{sono alzate}.
...
\iti{niente \itb{di} interessante}. \iti{Qualcosa \itb{di} bello}
...
The form is \textbf{Question word} + verb + subject:

\begin{itemize}[nosep]
    \item \iti{Dove sono andati i tuoi genitori?}
    \item \textit{Dove i tuoi genitori sono andati?} (Incorrect!)
\end{itemize}

\section{\iti{Chi} \textit{vs.} \iti{Che}}
\begin{itemize}[nosep]
    \item \itb{Chi} = who in questions / whoever (no noun before it)
    \item \itb{Che} = who/which/that in relative clauses (noun before it)
\end{itemize}

Examples: \iti{\itb{Chi} parla italiano?}: Who speaks Italian? (question), vs.
